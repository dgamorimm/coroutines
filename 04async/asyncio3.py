...
async def processar_dados(quantidade:int, dados:asyncio.Queue):
    print(f'Aguarde a leitura de {quantidade} dados...')
    processados = 0
    while processados < quantidade:
        await dados.get()
        processados += 1
        await asyncio.sleep(0.001)
    print(f'{quantidade} dados processados com sucesso...')
...
def main():
    ...
    tarefas = asyncio.gather(tarefa1, tarefa2, tarefa3)  # coletor, vai agrupar a stres tarefas e vai executar em paralelo
